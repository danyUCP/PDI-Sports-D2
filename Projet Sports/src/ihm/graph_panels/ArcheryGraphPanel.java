...
/**
 * <code>ArcheryGraphPanel </code> Is a class that implements the graphical ihm for adding,
 *  editing and deleting sessions for the archery sport in the database.
 * <strong>NOTE:</strong>
 *This class<code>ArcheryGraphPanel</code> implements all the components of the associated graphical ihm .
 *
...
public class ArcheryGraphPanel extends JPanel
{
	private User user;
	...
	public ArcheryGraphPanel(User user)
	{
		this.user = user;
	...
	{	
		...
		graphPanel.add(new ArcherySummaryBar(wm.getWorkoutList(6)), BorderLayout.CENTER);
	}
	
	public void initGraphChoice()
	{	
		title = new SportLabel("Tir à l'arc");
		...
		String[] choices = {"Mon résumé", "Ma précision"};
		...
		{			
			...
			if(e.getSource() == displayButton)
			{
				...
				if(choiceBox.getSelectedIndex() == 0)
					...
					graphPanel.add(new ArcheryPrecisionPie(wm.getWorkoutList(6)), BorderLayout.CENTER);
