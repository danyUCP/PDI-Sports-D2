...
/**
 * <code>RowingGraphPanel </code> Is a class that implements the graphical ihm for adding,
 *  editing and deleting sessions for the rowing sport in the database.
 * <strong>NOTE:</strong>
 *This class<code>RowingGraphPanel</code> implements all the components of the associated graphical ihm .
 *
...
public class RowingGraphPanel extends JPanel
{
	private User user;
	...
	public RowingGraphPanel(User user)
	{
		this.user = user;
	...
	{	
		...
		int choice=1;
        int choice2=0;
		graphPanel.add(new RowingSummaryBar(wm.getWorkoutList(3),choice,choice2), BorderLayout.CENTER);
	}
	
	public void initGraphChoice()
	{	
		title = new SportLabel("Rowing");
		...
		String[] choices = {"Durée", "Distance","Coups de paggaies"};
			...
			if(e.getSource() == displayButton)
			{
			     ...
			     if(choiceBox.getSelectedIndex() == 0) {
						...
						
					{
						...
						ArrayList<Workout>workoutList=wm.getWorkoutList(3);
						int size=workoutList.size();
						graphPanel.removeAll();
                        try {			
						graphPanel.add(new RowingSummaryBar(wm.getWorkoutList(3),size,choice2), BorderLayout.CENTER);
			     ...
			     if(choiceBox.getSelectedIndex() == 2) {
			    	 int choice2=2;
			     if( workoutNumberBox.getSelectedIndex() == 0)
