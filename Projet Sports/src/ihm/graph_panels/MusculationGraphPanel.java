...
import data.User;
import graph.MuscuBalancePie;
import graph.MuscuSummaryBar;
...
public class MusculationGraphPanel extends JPanel
{
	...
	public MusculationGraphPanel(User user)
	{
		this.user = user;
	...
	{	
		...
		graphPanel.add(new MuscuSummaryBar(wm.getWorkoutList(4)), BorderLayout.CENTER);
	}
	
	public void initGraphChoice()
	{	
		title = new SportLabel("Musculation");
		...
		String[] choices = {"Mon résumé", "Ma répartition"};
		...
		{			
			...
			if(e.getSource() == displayButton)
			{
				...
				if(choiceBox.getSelectedIndex() == 0)
					...
					graphPanel.add(new MuscuBalancePie(wm.getWorkoutList(4)), BorderLayout.CENTER);
